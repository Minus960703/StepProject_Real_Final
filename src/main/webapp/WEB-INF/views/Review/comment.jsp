...
<% } %>
	<!-- 반복 시작할 곳 -->
		<!-- DAO 처리 -->
		<!-- comment-no : <%//request.getParameter("no") %> -->
		<%
		...
		for(int i=0;i<dao.list.size();i++){
			...
			<%-- DAO 설정 시작--%>
					<% 
						...
						rdao=new RcommentDAO();
						rdao.setreviewcomment(Integer.parseInt((request.getParameter("no"))));
					...
					%>
						...
						<div class="comments">
							...
							<main><span>내용 : <%=rvo.getText() %></span></main>
